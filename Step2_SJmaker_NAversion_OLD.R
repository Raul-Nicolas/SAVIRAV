...
cigarcleaner <- function(cigarunsplitted){
  ...
  for(cont in 1: length(cigarspread)){
    if(is.na(as.numeric(cigarspread[cont]))){
      cigardf[row, 1] <- value
...
coordinateofcigarcalculator <- function(case_of_cigar, case_of_startread){
  ...
  beforeandsplicing = case_of_cigar[1:which(case_of_cigar$class == "N"),]
  
  Start = as.numeric(case_of_startread) + sum(as.numeric(beforeandsplicing$value[beforeandsplicing$class %in% countforcoordinates]))
  End = Start + as.numeric(beforeandsplicing$value[dim(beforeandsplicing)[1]])
  ...
  if(identical(magic, 'BAM\1')){
    ...
    if(length(N_ones)> 0){
      for(SJcont in 1: length(N_ones)){
        ...
        Global_SJ_coordinates = c(Global_SJ_coordinates, paste0(c(N_chrom[SJcont],coordinateofcigarcalculator(case_of_cigar, case_of_startread)), collapse = ":") )
